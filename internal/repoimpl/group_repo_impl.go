...
func (r *GroupRepo) GetByID(id uuid.UUID) (*dto.Group, error) {
	g := &dto.Group{}
	err := r.db.GetContext(context.Background(),
		g,
...
func (r *GroupRepo) List() ([]*dto.Group, error) {
	...
	err := r.db.SelectContext(context.Background(),
		&groups,
...
func (r *GroupRepo) AddUserToGroup(userID, groupID uuid.UUID) error {
	return util.WithTransaction(r.db, func(tx *sqlx.Tx) error {
		...
		return r.CheckResultRepo.CreateCheckResultTx(tx, cr)
	})
}
...
func (r *GroupRepo) RemoveUserFromGroup(userID, groupID uuid.UUID) error {
	return util.WithTransaction(r.db, func(tx *sqlx.Tx) error {
		...
		results, err := r.CheckResultRepo.GetCheckResultsByCheckID(checkID)
		if err != nil {
			return err
		}
		...
		for _, cr := range results {
			if cr.UserID == userID {
				if err := r.CheckResultRepo.DeleteCheckResult(cr.ID); err != nil {
					return err
				}
			}
		}

		_, err = tx.Exec(`DELETE FROM user_to_group WHERE user_id=$1 AND group_id=$2`, userID, groupID)
		return err
	})
}
